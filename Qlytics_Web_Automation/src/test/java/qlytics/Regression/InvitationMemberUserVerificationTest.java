...
public class InvitationMemberUserVerificationTest extends TestBase {
	...
	public void setUp() throws IOException {
		...
		logger = Logger.getLogger("InvitationMemberUserVerificationTest");
		System.out.println("InvitationMemberUserVerificationTestStarted");
	...
	public void testInvitationMemberUserVerification() throws Exception {
		String unique = AppLibrary.getFDate() + "_" + AppLibrary.randInt();
		String memEmail = "neoTestme_" + unique;
...
//		new LoginPage(appLibrary).Login("neoTest_05_02_20_18_34_277@mailinator.com", "Admin123!@#");
		...
		// Invite member
		new InvitationPage(appLibrary).fillInviteMemberForm(memEmail + "@mailinator.com", "members");

		// Verify invited admin in pending invitation

		AppLibrary.clickElement(driver, InvitationPage.pendingInvitationButton);

		new InvitationPage(appLibrary).verifyPendingInvitationMemberData(memEmail + "@mailinator.com",
				memEmail + "@mailinator.com", "MEMBERS", "No", "");

		AppLibrary.clickElement(driver, InvitationPage.closePendingButton);
		new HeaderPage(appLibrary).Logout();

		new MailinatorPage(appLibrary).openLink(memEmail, "Invitation");
		...
		new LoginPage(appLibrary).Login(memEmail + "@mailinator.com", Password);
		...
		new InvitationPage(appLibrary).verifyInvitationMemberData(Invitefirstname, Invitefirstname, InvitelastName,
				memEmail + "@mailinator.com", "Member");

		AppLibrary.clickElement(driver, InvitationPage.pendingInvitationButton);

		//verify Pending Invitation >> Accept invitation(Yes)
		new InvitationPage(appLibrary).verifyPendingInvitationMemberData(memEmail + "@mailinator.com",
				memEmail + "@mailinator.com", "MEMBERS", "Yes", "");
