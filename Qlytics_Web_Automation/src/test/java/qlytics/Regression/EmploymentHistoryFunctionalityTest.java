...
import qlytics.Beans.EmploymentHistoryBean;
...
public class EmploymentHistoryFunctionalityTest extends TestBase {
...
//		new LoginPage(appLibrary).Login(emailAddress + "@mailinator.com", Password);
		new LoginPage(appLibrary).Login("agammishra12@mailinator.com", "Admin123!@#");
		new HeaderPage(appLibrary).clickOnAccountSetting();
		AppLibrary.clickElement(driver, ProfilePage.profileButton);
		AppLibrary.waitUntilElementDisplayed(driver, ProfilePage.employmentHistoryButton);
		AppLibrary.clickElement(driver,ProfilePage.employmentHistoryButton);
		
		new ProfilePage(appLibrary).FillEmploymentHistoryForm("bpl", "engineer", "2018-04-25", "2019-04-25");
		EmploymentHistoryBean eh =new EmploymentHistoryBean();
//		eh.setEmploymentBean(companyVal, titleVal, fromVal, toVal, countryVal);
//		createProfile(pb);
//
//		verify(pb);
